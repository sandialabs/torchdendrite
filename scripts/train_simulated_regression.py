...
from torchdendrite.models import DendFCNetRegressor
from torchdendrite.utils import trainer, SimulatedRegression, plot_results
...
parser.add_argument(
    "--dataset",
    help="Which simulated data do you want?",
    required=True,
    choices=("simple_sinusoid", "linear"),
    type=str,
)
parser.add_argument(
    "--work_dir",
    help="Working Directory for results storage",
    default="work_dir",
    type=str,
)
parser.add_argument(
    "--resolution", help="Temporal resolution of dendrites", required=True, type=int
)
...
datasets = {
    "simple_sinusoid": SimulatedRegression.simple_sinusoid,
...
### Train Model ###
model, training_log, weight_log = trainer(
    model,
    train_set=train,
    test_set=test,
...
with torch.no_grad():
    y_pred = (
        model(
            torch.tensor(test.x).type(torch.FloatTensor).reshape(-1, 1).to(args.device)
        )
        .cpu()
        .numpy()
    )
